...
@spotify_data_router.get("/search")
async def search_spotify_tracks(
    ...
    if not access_token:
        raise HTTPException(status_code=401, detail="Spotify access token not found or expired.")
...
@spotify_data_router.get("/token")
async def get_spotify_access_token(auth: AuthJWT() = Depends()):
    user_id = auth.identity
    access_token = get_spotify_access_token_for_user(user_id)
    if not access_token:
        raise HTTPException(status_code=401, detail="Spotify access token not found or expired.")
